...
data <- data[, c("edad", "sexo", "ecivil", "Horas", "esc", "pobreza", "Sistema_alcantarillado", "numper", "ind_hacina", "indsan", "allega_ext", "allega_int", "Ingreso_persona")]
head(data)
summary(data)
...
#eliminar valores 0 de la variable ingreso_persona> data <- data[, c("edad", "sexo", "ecivil", "esc", "educ", "pobreza", "numper", "ind_hacina", "indsan", "allega_ext", "allega_int", "Ingreso_persona", "Ingreso_hogar")]
data <- data[data$Ingreso_persona > 0, ] # observaciones se reducen a 145,910

#estadisticas descriptivasstr
...
graf_esc_pre_impt <- ggplot(data = data) +  geom_histogram(aes(x = esc, y = after_stat(density)),
                                      bins = 30,
                                      fill = "lightblue",
                                      alpha = 0.9) +
  labs(
...
#realizar imputatción por el promedio
data$esc[which(is.na(data$esc))] = mean(data$esc, na.rm = TRUE)
table(is.na(data$esc)) # ya no hay NA's
summary(data$esc)
#gráfico post imputación
ggplot(data = data) +  geom_histogram(aes(x = esc, y = after_stat(density)),
                                      bins = 30,
                                      fill = "lightblue",
                                      alpha = 0.9) +
  labs(
...
gg_frecuencia_abs <- ggplot(data, aes(x = esc)) +
  geom_histogram(binwidth = 1, fill = "skyblue") +
  labs(title = "Años de escolaridad", x = "Escolaridad", y = "Frecuencia absoluta")

gg_frecuencia_abs
#al realizar esta imputación la disribución de los datos no se ve afectada, por lo que creo es adecuada

#variable pobreza. Tipo de dato chr / categorica
...
graf_probreza_pre_imputación <- ggplot(data, aes(x = pobreza)) +
  geom_bar(stat = "count", fill = "lightblue") +
  labs(x = "Niveles de pobreza", y = "Conteo")
graf_probreza_pre_imputación

################
### esta parte intento hacer la imputación a la variable pobreza, pero no lo he logrado

#convertir variables de chr a factor
data[c("sexo", "ecivil", "pobreza", "Sistema_alcantarillado", "ind_hacina", "indsan", "allega_ext", "allega_int")] <- 
  lapply(data[c("sexo", "ecivil", "pobreza", "Sistema_alcantarillado", "ind_hacina", "indsan", "allega_ext", "allega_int")], as.factor)

####################

# Crear la matriz de predictores
data$pobreza <- factor(data$pobreza, levels = c("No pobreza", "Pobreza extrema", "Pobreza no extrema"))

pred_matrix <- make.predictorMatrix(data)

# Excluir pobreza como predictor de sí misma
pred_matrix["pobreza", "pobreza"] <- 0

#realizar imputación
imputacion <- mice(data, method = "polyreg", predictorMatrix = pred_matrix, m = 5)
# Obtener el conjunto de datos imputado
data_1<- complete(imputacion)

#4)
#creación variable objetivo 40_superior